...
RcppExport SEXP TwoPhase_GeneralSpline (SEXP Y_R, SEXP X_R, SEXP Z_R, SEXP W_R, SEXP Bspline_R, SEXP hn_R, SEXP MAX_ITER_R, SEXP TOL_R, SEXP noSE_R) 
{
	/*#############################################################################################################################################*/
	...
	const MapMatd W(as<MapMatd>(W_R));
	...
	const int Z_nc = Z.cols(); // number of inexpensive covariates Z
	const int X_nc = X.cols(); // number of expensive covariates X
	const int W_nc = W.cols(); // number of inexpensive covariates W
	const int ncov = X_nc+Z_nc+W_nc; // number of all covariates
	const int s = Bspline.cols(); // number of B-spline functions
	const int theta_W_pos = X_nc+Z_nc; // the position of the first covariate in W
	/**** some useful constants ********************************************************************************************************************/
	/*#############################################################################################################################################*/	
	...
	LS_XtY_static.segment(X_nc,Z_nc) = Z.transpose()*Y;
	LS_XtY_static.tail(W_nc) = W.transpose()*Y;
	
	// t(X,Z,W)*(X,Z,W)
	MatrixXd LS_XtX_static(ncov, ncov); 
	LS_XtX_static.topLeftCorner(X_nc,X_nc) = X.transpose()*X;
	LS_XtX_static.block(0,X_nc,X_nc,Z_nc) = X.transpose()*Z.topRows(n2);
	LS_XtX_static.topRightCorner(X_nc,W_nc) = X.transpose()*W.topRows(n2);
	LS_XtX_static.block(X_nc,X_nc,Z_nc,Z_nc) = Z.transpose()*Z;
	LS_XtX_static.block(X_nc,theta_W_pos,Z_nc,W_nc) = Z.transpose()*W;
	LS_XtX_static.bottomRightCorner(W_nc,W_nc) = W.transpose()*W;
	LS_XtX_static.block(theta_W_pos,X_nc,W_nc,Z_nc) = LS_XtX_static.block(X_nc,theta_W_pos,Z_nc,W_nc).transpose();
	...
	for (iter=0; iter<MAX_ITER; iter++) 
	{
		...
		for (int i=0; i<n_minus_n2; i++) {
			for (int k=0; k<m; k++) {
				LS_XtX.block(0,X_nc,X_nc,Z_nc).noalias() += q(i,k)*X_uni.row(k).transpose()*Z.row(i+n2);
				LS_XtX.topRightCorner(X_nc,W_nc).noalias() += q(i,k)*X_uni.row(k).transpose()*W.row(i+n2);
				LS_XtY.head(X_nc).noalias() += q(i,k)*X_uni.row(k).transpose()*Y(i+n2);				
			}
		}
		
		LS_XtX.block(X_nc,0,Z_nc,X_nc) = LS_XtX.block(0,X_nc,X_nc,Z_nc).transpose();
		LS_XtX.bottomLeftCorner(W_nc,X_nc) = LS_XtX.topRightCorner(X_nc,W_nc).transpose();
	...
	else 
	{
		...
		loglik = WaldLinearGeneralSplineProfile(pB, p_col_sum, q_row_sum, p, p0, P_theta, q, resi_n, logp, theta, Y, X, Bspline_uni, Z, W, X_uni,
			X_uni_ind, Bspline_uni_ind, p_static, sigma_sq, n, n2, m, s, n_minus_n2, X_nc, Z_nc, W_nc, MAX_ITER, TOL);
		if (loglik == -999.) 
		{
			flag_nonconvergence_cov = true;
		}
		...
		for (int i=0; i<ncov; i++) 
		{
			...
			theta0(i) += hn;
			sigma_sq0 = sigma_sq;
			profile_vec(i) = WaldLinearGeneralSplineProfile(pB, p_col_sum, q_row_sum, p, p0, P_theta, q, resi_n, logp, theta0, Y, X, Bspline_uni, Z, W, X_uni,
				X_uni_ind, Bspline_uni_ind, p_static, sigma_sq0, n, n2, m, s, n_minus_n2, X_nc, Z_nc, W_nc, MAX_ITER, TOL);
		}
		theta0 = theta;
		sigma_sq0 = sigma_sq+hn;	
		profile_vec(ncov) = WaldLinearGeneralSplineProfile(pB, p_col_sum, q_row_sum, p, p0, P_theta, q, resi_n, logp, theta0, Y, X, Bspline_uni, Z, W, X_uni,
			X_uni_ind, Bspline_uni_ind, p_static, sigma_sq0, n, n2, m, s, n_minus_n2, X_nc, Z_nc, W_nc, MAX_ITER, TOL);
		for (int i=0; i<ncov+1; i++) 
		{
		...
		for (int i=0; i<ncov; i++) 
		{
			...
			theta0(i) += hn;
			sigma_sq0 = sigma_sq+hn;
			loglik = WaldLinearGeneralSplineProfile(pB, p_col_sum, q_row_sum, p, p0, P_theta, q, resi_n, logp, theta0, Y, X, Bspline_uni, Z, W, X_uni,
				X_uni_ind, Bspline_uni_ind, p_static, sigma_sq0, n, n2, m, s, n_minus_n2, X_nc, Z_nc, W_nc, MAX_ITER, TOL);
			if (loglik == -999.) 
			{
				flag_nonconvergence_cov = true;
...
RcppExport SEXP TwoPhase_MLE0 (SEXP Y_R, SEXP X_R, SEXP W_R, SEXP MAX_ITER_R, SEXP TOL_R, SEXP noSE_R)
/**** when there is no Z ***************************************************************************************************************************/
{
	/*#############################################################################################################################################*/
	/**** pass arguments from R to cpp *************************************************************************************************************/
	...
	const int X_nc = X.cols(); // number of expensive covariates X
	const int W_nc = W.cols(); // number of inexpensive covariates W
	const int ncov = X_nc+W_nc; // number of all covariates
	/**** some useful constants ********************************************************************************************************************/
	/*#############################################################################################################################################*/	
	...
	{
		WaldLinearVarianceMLE0(cov_theta, LS_XtX, LS_XtY, p, Y, W, theta, X_uni, q, n_minus_n2, X_nc, W_nc, m, ncov, n, n2, sigma_sq); 
	}
	/**** variance estimation **********************************************************************************************************************/
	/*#############################################################################################################################################*/
